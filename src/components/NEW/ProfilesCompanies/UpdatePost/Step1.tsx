...
const Step1: React.FC<{
  ...
  useEffect(() => {
    if (item !== null) {
      step1.setValue("title", item.title);
      Object.entries(DEGREE_CHOICES).map(([key, value]) => {
        if (key === item.degree) {
          step1.setValue("degree", value);
          console.log(value);
        }
      });
      Object.entries(SEX_CHOICES).map(([key, value]) => {
        if (key === item.sex) {
          step1.setValue("gender", value);
          console.log(value);
        }
      });
      Object.entries(SARBAZI_CHOICES).map(([key, value]) => {
        if (key === item.sarbazi) {
          step1.setValue("military", value);
          console.log(value);
        }
      });
      Object.entries(SALARY_CHOICES).map(([key, value]) => {
        if (key === item.salary) {
          step1.setValue("salary", value);
          console.log(value);
        }
      });
      Object.entries(COOPERATION_CHOICES).map(([key, value]) => {
        if (key === item.cooperation_type) {
          step1.setValue("type", value);
          console.log(value);
        }
      });
      Object.entries(EXPERIANCE_CHOICES).map(([key, value]) => {
        if (key === item.experience) {
          step1.setValue("work", value);
          console.log(value);
        }
      });
    }
    ...
    >
      ...
      >
        ...
        <Select
          ...
          defaultValue={item !== null ? step1.getValues("type") : ""}
          label="نوع همکاری"
